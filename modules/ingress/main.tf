locals {
  ...
  annotations = {
    ...
    "alb.ingress.kubernetes.io/load-balancer-attributes" = var.enable_send_alb_logs_to_s3 ? "access_logs.s3.enabled=true,access_logs.s3.bucket=${local.alb_log_bucket_name}" : var.load_balancer_attributes
